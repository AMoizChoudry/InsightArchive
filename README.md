...
* **Infrastructure Transparency:** Provides full visibility into the chunking, embedding, and retrieval process.
* **Granular Performance Profiling:** Enables the precise measurement of **Database Retrieval Latency** vs. **LLM Generation Latency**.
* **Vector Database Control:** Utilizing **LanceDB** locally provides superior control over data indexing and significantly reduces costs.
...
* **Vector Search (`text-embedding-3-small`):** Captures semantic meaning and conceptual queries.
* **BM25 Keyword Search:** Ensures exact matches for technical terms, part numbers, and identifiers.
...
* **Agentic Tool Calling:** The LLM autonomously decides when to query the knowledge base.
...
### 1. Install `uv`
This project uses `uv` for high-performance dependency management. You must install it first:
